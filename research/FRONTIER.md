...
| 3d | **Generalize veil-boundary integration** | F78 closes the last visible `_010` seam by physically inverting foreground radiance/rear transmission at the 10% aperture-coverage contour, selecting the local PSF by two-frame forward fit, and limiting exterior-trend inference to proposal-specific below-noise cases. | 🔶 VISUALLY FROZEN on the `s29_002/007/010` quick cohort; `_010` right-side output at `bf99365` is user-validated near-perfect. Next: freeze parameters, generate a fresh post-rule split, cross disk/box/compound PSFs, noise/ISP and real capture, and require protected-region identity plus no new contour/false-texture tail. Do not tune further on `_010`. |
