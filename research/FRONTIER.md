...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F70 / SAFETY-DISABLED: V2 generator formation passes hidden-background invariance on all six compact S27 scenes. The preserved solver is now paired with discrete support; connected focused graph regions repair split objects, while native cross-frame satellite graphs hard-select tiny fragments and leave unresolved uncertainty rear-veto-only. S27 improves MAE/MSE and every physical partition 6/6, has zero rear overlap in hard/core/boundary/far regions, exact far identity, and mean silhouette IoU 0.97766. S25 preservation remains intact. Freeze now; S27 shaped the rule, so a genuinely unseen V2 split is required before promotion. Inspector remains frozen until the end. |
