...
| 7 | Alignment and focus breathing | ACTIVE | F81 ships depth-binned local transforms with valley bin edges, a stretch-limited blended field, and single-resample composition: +0.0906 GT-SSIM on an isolated parallax factory, 1.5-2.2x lower near-content residual on all three moving phone sweeps, zero-motion left alone. Remaining: the alternating motion/depth/calibration estimator (`depth_model="joint"`) registers moving sweeps best but invents motion on still ones, and its observation model explains only 25-50% of tile shifts. Next: a better observation model (occlusion-aware tiles, per-tile depth confidence) before the joint path can be non-regressing; disocclusion bands still have no per-pixel validity route into fusion. |
