...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F68 / SAFETY-DISABLED: the full S26 audit exposed six residual opaque-core rear leaks and one false factory verdict. `s26_020`'s apparent veil regression is a 17,299-pixel semantic hole through a visibly solid foreground tabletop, not an operator error. New S27 sources reject large enclosed holes and fill only tiny segmentation speckles. The F67 solver remains preserved while a separate edge-respecting ownership matte is developed; possible-front evidence may veto rear synthesis but cannot silently become hard geometry. S25 remains the stable solver carpet. Generate S27 only after the boundary rule freezes; inspector remains frozen until the end. |
