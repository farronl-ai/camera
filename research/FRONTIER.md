...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F65 / SAFETY-DISABLED: primary S25 formation and the paired solver now enforce zero rear throughput throughout opaque foreground support while allowing foreground-only spread outward. A formation-native, cross-frame-corroborated owner selector gives 6/6 direct-error and exterior-veil wins with exact far-background identity; 5/6 improve SSIM/core. The single tiny core dissent must be localized without ever blending hidden rear detail inward. Inspector stays at the preserved S23 checkpoint; no S26 until the rule freezes. |
