...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F72 / SAFETY-DISABLED: S28 formation passes 12/12. Rear-focused connected graphs now carve semantic rear openings; a strict native front-directed graph hard-selects missed opaque fragments; sub-quantization rear tails are zeroed. S27 geometry is unchanged and S27/S28 both have zero rear application in hard/core/soft-edge/boundary/far regions. Five causal S28 solves improve every physical partition with exact far identity; 67 tests pass. Full S28, preservation cohorts, and a new post-rule split remain before the one-time inspector rebuild. |
