...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F69 / SAFETY-DISABLED: F68 repaired ambiguous source topology, but F69 proved the V1 renderer itself still leaked hidden rear content across the antialiased part of hard foreground support—7,397/55,222 pixels on the actual diagnosed bird source. V2 separates binary ownership from display alpha: hidden-background influence is exactly zero over all owned pixels while foreground-only outward spread remains. S27 is now the first V2 cohort; its one-scene sentinel passes the formation invariant. Validate a compact S27 carpet before returning to the preserved F67 solver, then infer hard ownership and rear permission separately. Inspector remains frozen until the end. |
