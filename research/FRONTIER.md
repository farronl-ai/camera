...
| 7 | Alignment and focus breathing | ACTIVE | F81 ships depth-binned local transforms; F82 adds per-pixel refusal of parallax-uncovered scene (analytic factory 0.8754 -> 0.9660 -> 0.9785 GT-SSIM). Remaining: the refusal is two-sided because front/back order at each boundary is unknown, so it discards the occluding side too; occlusion-edge blur is the standard cue for that ordering (Marshall 1996, already listed below) and would also sharpen boundary localization, which depth-from-focus gives only softly. The joint estimator still invents motion on still stacks. |
