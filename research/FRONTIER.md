...
| 7 | Alignment and focus breathing | ACTIVE | F81 depth-binned local transforms; F82 per-pixel refusal of parallax-uncovered scene (factory 0.8754 -> 0.9660 -> 0.9785 GT-SSIM). F83 closed the front/back-ordering question as a rigorous negative: the occlusion-edge-blur cue works (contour localization 32 px -> 4.8 px, ordering bit voted correctly, front mask 91.1%) but one-sided refusal loses to two-sided, because a defocused occluder's own matte mixes background onto its boundary. Next: the joint estimator still invents motion on still stacks; and per-pixel validity currently cannot express partial confidence, only refusal. |
