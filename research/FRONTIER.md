...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F63 / SAFETY-DISABLED: F62's front-first repair remains valid, but promotion is paused for a formation-taxonomy reset. Primary opaque-core, slender/all-veil opaque stress, true material transmission, and malformed-overlay robustness must not be pooled. Regrade F62 by regime before returning to its small positive fine-band tail. |
