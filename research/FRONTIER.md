...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F64 / SAFETY-DISABLED: the reweighted S23 judge fires only on primary opaque-core scenes. Local 75% owner-mask consensus repairs its causal far-background leak; 7/7 fires now improve SSIM/MAE/MSE and all partitions, with 65 exact refusals. S23 is development evidence. Auto stays off because all seven retain a positive fine-band tail (+0.0011…+0.0042); no new large split until that mechanism freezes. |
