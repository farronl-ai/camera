...
| 16d | **Coarse-band regional veil correction (WIDE occluders)** | The scale-split (F27/F39 + Farron): fine contours = reconstruction's domain (no strong veil there, by geometry); veil haze = LOW-frequency regional field over wide fringes — correct it in COARSE pyramid bands only (high SNR at coarse scale evades F27's noise-amplification; fine bands untouched). Slots into perband's per-band architecture. FIRST: wide-occluder benchmark scene — measure the haze error before building (E0 discipline; current benches are thin-structure-heavy). | ⬜ |
