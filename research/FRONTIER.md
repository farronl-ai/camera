...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F67 / SAFETY-DISABLED: S26 disproved the first freeze and became development evidence. The preserved focused-RGB completion now admits only cross-frame-proven satellites; rear application additionally requires causal focal-direction ordering plus reverse-reblur residual above its measured noise floor. Re-frozen S25: zero rear pixels in GT core/edge/far for 6/6; 6/6 MAE/MSE/core/veil wins and exact far identity; mean silhouette IoU 0.9647; 5/6 SSIM wins. S26 sentinels 013/014 now have zero rear core/edge/far overlap and all direct metrics improve, but 014 still exposes a hard-front far extension. Finish boundary attribution, then validate on a genuinely new post-freeze split; inspector remains frozen until the end. |
