...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F71 / SAFETY-DISABLED: fresh S28 proves V2 formation on 12/12 scenes (zero hidden-background dependence in hard ownership, unit owned coverage, exterior veil retained), but invalidates inverse promotion. One semantic proposal imports 49,983 rear pixels into model ownership, two scenes leak rear correction into hard ownership, and one reaches far background; two ambiguous scenes correctly refuse. Preserve F70's solver. Next infer or refuse local rear openings from independent focal-order regions, then repair the narrow rear-mask leaks and generate another frozen split. Inspector remains frozen until the end. |
