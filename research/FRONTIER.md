...
| 3b | **Occlusion-AWARE scene recovery** | Joint layer inversion (de-veiling beyond camera-mix). | 🔶 F74 / SAFETY-DISABLED: S28 formation passes 12/12. Rear-connected focal graphs carve openings only inside a cross-frame-corroborated object; front-directed native graphs hard-select fragments; sub-quantization rear tails are zeroed. Focused-owner smoothing and edge preservation use only that same observation. Full S28 has ten fires and two exact refusals; all ten improve MAE/MSE and every physical partition, with zero rear application in protected regions and exact far identity. S27/S25 preservation retains zero protected-region rear overlap. Freeze and validate S29 before the one-time inspector rebuild. |
