...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F74) — F68/F69 repair opaque source topology and discrete formation support; F70 preserves the numerical inverse while selecting focused graph ownership. F72 treats rear-connected focal graphs as negative geometry, front-directed native fragments as discrete focused-owner choices, and sub-quantization tails as identity. F73 keeps denoising and edge preservation within one focused-owner observation. F74 requires the enclosing object itself to be cross-frame corroborated before negative ownership can remove support. Full S28 improves MAE/MSE and every partition for all ten fires; S27/S25 preservation has zero protected-region rear overlap. S29, shipping path, and the one-time final inspector remain open. |
