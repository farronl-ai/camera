...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F72) — F68/F69 repair opaque source topology and discrete formation support; F70 preserves the numerical inverse while selecting focused graph ownership. Fresh S28 then exposed semantic rear openings, missed native fragments, and low-weight rear tails. F72 treats rear-connected focal graphs as negative geometry, front-directed native fragments as discrete focused-owner choices, and sub-quantization tails as identity. S27 is unchanged; S27/S28 have zero rear application in hard/core/soft-edge/boundary/far regions, and five causal S28 solves improve every physical partition with exact far identity. Full S28, preservation, a post-rule split, shipping path, and the one-time final inspector remain open. |
