...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F67) — F60 replaced the invalid V1 judge; F61/F62 established asymmetric rear visibility and front-first geometry; F63 split formation regimes; F64 made confidence local; F65 corrected primary opaque formation; F66 separated completed front geometry from rear permission. F67 preserves that solver while requiring causal focal-direction evidence and reverse-reblur presence before rear application. Re-frozen S25 has zero rear-mask overlap with GT core, antialiased edge, and far background in 6/6, plus 6/6 MAE/MSE/core/veil gains and exact far identity (5/6 SSIM). S26 exposed and shaped the repair, so it is development evidence; the S23 inspector and shipping auto path stay preserved/disabled until a new post-freeze split passes and the final inspector is generated once. |
