...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F69) — F60 replaced the invalid V1 judge; F61/F62 established asymmetric rear visibility and front-first geometry; F63 split formation regimes; F64 made confidence local; F66/F67 preserved the solver while separating hard front geometry from rear permission. F68 removed semantic holes masquerading as apertures. F69 then proved F65's renderer still certified only an eroded `alpha==1` subset: the actual `extension_007` source changed 7,397 hard-owned pixels when only hidden rear content changed. `one_sided_opaque_v2` now stores pre-antialias binary ownership separately and makes rear throughput exactly zero over all of it while retaining foreground-only outward spread. S25/S26 remain historical development evidence; S27 is the first valid V2 cohort. S27 validation, inverse ownership, shipping, and the one-time final inspector remain open. |
