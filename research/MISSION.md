...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F68) — F60 replaced the invalid V1 judge; F61/F62 established asymmetric rear visibility and front-first geometry; F63 split formation regimes; F64 made confidence local; F65 corrected primary opaque formation; F66 separated completed front geometry from rear permission; F67 preserved the solver and added causal rear gates. The full S26 audit then exposed six residual geometry leaks and a more fundamental judge defect: a semantic hole through a visibly solid tabletop was labeled as target-background GT. S27 now requires unambiguous solid-opaque source topology, filling only tiny speckles and excluding substantial holes for later aperture/transmission work. S25 remains the stable solver carpet; the ownership boundary, S27 validation, shipping path, and one-time final inspector remain open. |
