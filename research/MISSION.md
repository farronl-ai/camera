...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F70) — F68 removed semantic holes masquerading as apertures; F69 proved V1 still leaked rear content across the non-unit part of source ownership and introduced V2's separate binary support. F70 pairs the inverse forward/adjoint with that support, uses strictly observed connected focal graph regions to repair split foregrounds, and resolves tiny cross-frame satellites at native scale. Compact S27 improves MAE/MSE and every physical partition 6/6 with zero rear overlap in hard/core/boundary/far regions and exact far identity; S25 preservation remains intact. S27 shaped F70, so a frozen unseen V2 validation split, shipping path, and one-time final inspector remain open. |
