...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F75) — F68/F69 repair opaque source topology and discrete formation support; F70 preserves the numerical inverse while selecting focused graph ownership. F72–F74 separate rear openings, native fragments, focused-owner radiance, and corroborated negative evidence. F75 hard-selects tiny graph continuations whose focal direction was swamped by sharp rear surroundings, while removing only unresolved source-mask speckle from new factory truth. S29 formation and blind geometry pass with zero protected-region rear application; full solve, shipping path, and the final inspector remain open. |
