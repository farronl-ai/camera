...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F73) — F68/F69 repair opaque source topology and discrete formation support; F70 preserves the numerical inverse while selecting focused graph ownership. F72 treats rear-connected focal graphs as negative geometry, front-directed native fragments as discrete focused-owner choices, and sub-quantization tails as identity. F73 keeps denoising and edge preservation within one focused-owner observation. Full S28 has ten fires and two exact refusals; every fire improves MAE/MSE and every physical partition, rear application is zero in protected regions, and far background is exact. S28 shaped the last rule, so preservation, a post-rule split, shipping path, and the one-time final inspector remain open. |
