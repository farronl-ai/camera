...
|---|---|
| F27: de-veiling by inversion loses | **CONDITIONALLY REOPENED / RESEARCH, AUTO DISABLED** (F65) — F60 replaced the invalid V1 judge; F61/F62 established asymmetric rear visibility and front-first geometry; F63 split formation regimes; F64 made confident ownership local. F65 corrects the primary opaque formation itself: hidden rear detail has zero throughput throughout latent foreground support, while foreground radiance may spread outward. The paired S25 research path improves direct error and exterior veil in 6/6 cases with exact far-background identity; one tiny foreground-core/SSIM dissent remains. The S23 inspector and shipping runtime stay preserved/disabled until that cause, the fine-band tail, explicit boundary audits, and a genuinely post-freeze split pass. |
